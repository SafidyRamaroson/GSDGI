...
export const CircleStep: React.FC<CircleStepProps> = ({
  ...
  const checkStepTo = useStepStore((state) => state.checkStepTo);
  ...
  return (
    <div className="flex flex-row items-center justify-start gap-4 mt-6 w-full">
      <button
        ...
        } border-2 border-green-500 w-10 h-10 rounded-full flex items-center justify-center cursor-pointer transition-colors duration-300`}
        onClick={() => checkStepTo(no)}
      >
        {isActive ? (
          <svg
            width="21"
